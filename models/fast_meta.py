...
class LabeledImageDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        img = Image.open( self.images[idx] )
        label = torch.load( self.images[idx].replace(".png", ".pt") )
        if self.transform:
            img = self.transform(img)
        return img, label
...
class ImagePool(object):
    ...
    def add(self, imgs, targets=None):
        ...
        output_filename = os.path.join( self.root, "%d"%(self._idx))
        for idx, label in enumerate(targets):
            torch.save(label, output_filename+'-%d.pt'%(idx))
        self._idx+=1
