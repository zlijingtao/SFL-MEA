...
def get_dali_train_loader(dali_cpu=False):
    def gdtl(data_path, batch_size, workers=5, _worker_init_fn=None, data_proportion = 1.0):
        if torch.distributed.is_initialized():
            local_rank = torch.distributed.get_rank()
            world_size = torch.distributed.get_world_size()
        else:
...
def get_pytorch_train_loader(data_path, batch_size, workers=5, _worker_init_fn=None, input_size=224, data_portion = 1.0):
    traindir = os.path.join(data_path, 'train')
