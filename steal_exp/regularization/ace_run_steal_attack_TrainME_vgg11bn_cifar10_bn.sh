                        ...
                        for attack_style in $attack_style_list; do
                                for num_query in $num_query_list; do
                                        for data_proportion in $data_proportion_list; do
                                                ...
                                                filename=ace_V2_epoch_vgg11_bn_cutlayer_${cutlayer}_client_1_seed125_dataset_${dataset}_lr_0.05_gan_adv_step3_both_res_normN4C64_${regularization_strength}_200epoch_bottleneck_noRELU_C8S1_ssim_0.5
                                                ...
                                                CUDA_VISIBLE_DEVICES=${GPU_id} python main_model_steal.py   --arch=${arch} --cutlayer=$cutlayer --batch_size=${batch_size} \
                                                        ...
                                                        --attack_style=$attack_style --bottleneck_option ${bottleneck_option}  --data_proportion=${data_proportion} --train_clas_layer=${train_clas_layer}
                                                done
                                        done
                                done
                        done
