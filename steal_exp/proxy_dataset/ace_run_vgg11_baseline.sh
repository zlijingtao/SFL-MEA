...
folder_name="poison_saves/proxy_dataset"
bottleneck_option=None
gan_loss_type=SSIM
# dataset_portion_list="1.0 0.1"
dataset_portion_list="0.1"
        ...
        for random_seed in $random_seed_list; do
                for regularization_strength in $regularization_strength_list; do
                        for cutlayer in $cutlayer_list; do
                                for dataset_portion in $dataset_portion_list; do
                                        filename=ace_${scheme}_${arch}_cutlayer_${cutlayer}_client_${num_client}_seed${random_seed}_dataset_${dataset}_lr_${learning_rate}_${regularization}_both_${train_gan_AE_type}_${regularization_strength}_${num_epochs}epoch_dataset_portion_${dataset_portion}
                                        CUDA_VISIBLE_DEVICES=${GPU_id} python main_MIA.py --arch=${arch}  --cutlayer=$cutlayer --batch_size=${batch_size} \
                                                ...
                                                --local_lr $local_lr --bottleneck_option ${bottleneck_option} --folder ${folder_name} --dataset_portion ${dataset_portion}
